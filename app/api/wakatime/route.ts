...
function secondsToText(seconds: number) {
  ...
  return rest ? `${hours} hr${hours === 1 ? "" : "s"} ${rest} min${rest === 1 ? "" : "s"}` : `${hours} hr${hours === 1 ? "" : "s"}`
}
...
function aggregateNamed(days: AnyRecord[], key: string, limit = 12) {
  ...
  const totalSeconds = [...map.values()].reduce((sum, item) => sum + item.total_seconds, 0)
...
function aggregateAi(days: AnyRecord[], projects: AnyRecord[], editors: AnyRecord[]) {
  ...
  return {
    ...
    promptLengthAvg: totals.promptEvents ? totals.promptLengthSum / totals.promptEvents : 0,
    projectBreakdown: projects
      .filter((item) => numberValue(item.ai_additions) + numberValue(item.ai_deletions) + numberValue(item.ai_agent_line_changes) > 0)
      .slice(0, 5),
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const cacheKey = `wakatime-${apiKey.slice(0, 8)}-${range.start}-${range.end}`
    const now = Date.now()
    ...
    const days: AnyRecord[] = Array.isArray(summariesPayload?.data) ? summariesPayload.data : []
    const allTime = allTimePayload?.data || {}

    const recentSeconds = days.reduce((sum: number, day: AnyRecord) => sum + numberValue(day?.grand_total?.total_seconds), 0)
    const activeDays = days.filter((day: AnyRecord) => numberValue(day?.grand_total?.total_seconds) > 0)
    const dailyAverageSeconds = activeDays.length ? recentSeconds / activeDays.length : 0
    const bestDay = activeDays.reduce((best: AnyRecord | null, day: AnyRecord) => {
      if (!best) return day
      return numberValue(day?.grand_total?.total_seconds) > numberValue(best?.grand_total?.total_seconds) ? day : best
    }, null)
    ...
    const responseData = {
        ...
        bestDay: bestDay
          ? {
              ...
              text: secondsToText(numberValue(bestDay.grand_total?.total_seconds)),
            }
          : null,
        range: {
          start: range.start,
